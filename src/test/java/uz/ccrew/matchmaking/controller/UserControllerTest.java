...
public class UserControllerTest {
    ...
    void getUser() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/user/get")
                        .header("Authorization", "Bearer " + ACCESS_TOKEN) // Set your authorization header here
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.data").exists())
                .andExpect(MockMvcResultMatchers.jsonPath("$.data.id").isNotEmpty())
                .andExpect(MockMvcResultMatchers.jsonPath("$.data.login").value("Azimjon"))
    ...
    void updateUser() throws Exception {
        UserUpdateDTO updateDTO = new UserUpdateDTO("Nazarov", "200622Az", UserRole.SERVER);
    ...
    void deleteUser() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.delete("/api/v1/user/delete")
                        .header("Authorization", "Bearer " + ACCESS_TOKEN)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.status().isOk())
    ...
    void getUserList() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/user/get/list")
                ...
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.data").exists())
                .andExpect(MockMvcResultMatchers.jsonPath("$.data.content").exists()) // Adjust as per your Page<UserDTO> structure
                .andDo(print());
    }

    public String getAdminAccessToken() {
        User user = User.builder().login("admin").password(passwordEncoder.encode("200622az")).role(UserRole.ADMINISTRATOR).build();
